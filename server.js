...
mongoclient.connect('mongodb://ujjwalobs:ujjwalkr@ds239965.mlab.com:39965/online_banking_system',function(err,db1){
    ...
    app.listen(0223,function(){
        console.log("connected to port: 0223");	
    });
});
...
app.use(function (req, res, next) {
  ...
  var cookie = req.cookies.cookieuser;
  if (cookie === undefined)
  {
    // no: set a new cookie
...
var sessionChecker = (req, res, next) => {
    if (req.session.user && req.cookies.user_sid) {
        res.redirect('/wthtml/homepagetest.html');
    } else {
        next();
    }    
};
        ...
        if(result){
            ...
            db.collection('user').findOne(old_user,function(err,success){
                ...
                if(success){
                    ...
                    req.cookies.user_sid = post.user_id;
